                  ...
                  <div key={caseItem.id} className="card glass-hover" style={{
                    ...
                    <div style={cardBodyStyle}>
                      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 16, alignItems: 'start' }}>
                        <div style={{ fontSize: 28, lineHeight: 1, paddingTop: 4 }}>⚖️</div>
                        <div style={{ minWidth: 0 }}>
                          <h3 style={{ fontSize: 18, fontWeight: 800, color: '#fff', lineHeight: 1.35, marginBottom: 8, overflowWrap: 'anywhere' }}>
                            {caseItem.title}
                          </h3>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center', marginBottom: 10 }}>
                            <span style={pillStyle('#60a5fa')}>📑 {caseItem.citation}</span>
                            <span style={pillStyle('#64748b')}>🏛️ {caseItem.court}</span>
                            <span style={pillStyle('#64748b')}>📅 {caseItem.year}</span>
                          </div>
                        </div>
                        <span style={{
                          ...pillStyle('#60a5fa'),
                      ...
                      <div style={{
                        background: 'rgba(30,41,59,0.5)', borderRadius: 12, padding: 16,
                        marginTop: 16, marginBottom: 16,
                      }}>
                        <p style={{ color: '#cbd5e1', fontSize: 14, lineHeight: 1.7, margin: 0 }}>{caseItem.summary}</p>
                      </div>
                  ...
                  <div key={statute.id} className="card glass-hover" style={{
                    ...
                    <div style={cardBodyStyle}>
                      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 14, alignItems: 'start' }}>
                        <span style={{ fontSize: 30, flexShrink: 0, lineHeight: 1, paddingTop: 2 }}>📜</span>
                        <div style={{ minWidth: 0 }}>
                          <h3 style={{ fontSize: 16, fontWeight: 800, color: '#fff', marginBottom: 8, lineHeight: 1.35, overflowWrap: 'anywhere' }}>{statute.title}</h3>
                          <span style={pillStyle('#34d399')}>{statute.sections}</span>
                        </div>
                      </div>
                      <div style={{
                        background: 'rgba(30,41,59,0.5)', borderRadius: 12, padding: 14, margin: '16px 0 14px',
                      }}>
                        <p style={{ color: '#cbd5e1', fontSize: 13, lineHeight: 1.65, margin: 0 }}>{statute.description}</p>
                      </div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 }}>
                        {statute.keywords.split(',').map((keyword, idx) => (
                          <span key={idx} style={pillStyle('#6ee7b7')}>{keyword.trim()}</span>
                        ))}
                      </div>
                    </div>
                    <div style={{ padding: '0 24px 20px', display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 10 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {searchResults.map((term, idx) => (
                  <div key={idx} className="card glass-hover" style={{
                    ...
                    <div style={cardBodyStyle}>
                      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 14, alignItems: 'start', marginBottom: 16 }}>
                        <span style={{ fontSize: 28, lineHeight: 1, paddingTop: 2 }}>📖</span>
                        <div style={{ minWidth: 0 }}>
                          <h3 style={{ fontSize: 20, fontWeight: 800, color: '#fff', lineHeight: 1.35, margin: 0, overflowWrap: 'anywhere' }}>{term.term}</h3>
                        </div>
                        <span style={{ ...pillStyle('#c084fc'), justifySelf: 'end' }}>{term.category}</span>
                      </div>
                      <div style={{
                        background: 'rgba(30,41,59,0.5)', borderRadius: 12, padding: 18, marginBottom: 16,
                      }}>

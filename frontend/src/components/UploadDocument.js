...
const UploadDocument = ({ onDraftGenerated }) => {
  ...
  const getErrorMessage = (err, fallbackMessage) => {
    ...
    if (status === 413) {
      return details || 'File is too large. Maximum size is 20MB.';
    }
    if (status === 429 || code === 'GEMINI_RATE_LIMIT') {
      return details || 'Gemini is currently rate-limited. Please retry in a moment.';
    }
    if (code === 'GEMINI_AUTH_ERROR') {
      return details || 'Gemini authentication failed. Please contact the administrator.';
    }
